...
python ${script} -ff CHARMM -c test_s5s6_nbfix.cfg -ts "2nd Shell Na+ SF Coordination" > NAVAB_XTAL_PD-S5S6-1kcal-CHARMM_WT_SOD150_macroevents.txt
python ${script} -ff OPLS -c testPD_s5s6_opls.cfg -ts "1st Shell Na+ SF Coordination" > NAVAB_XTAL_PD-S5S6-1kcal-OPLS_WT_SOD150_macroevents.txt
"""
...
if __name__ == '__main__':
    ...
    for series_num, series_name in enumerate(args.series_name):
        ...
        for transr in trans.iterrows():
            row = transr[1]
            start = row["OccMacrostate0_Start"]
            end = row["OccMacrostate2_Start"]

            if start == "2'" and end == "2":
                print(str(row["TrajNum"])+" BACK "+row["OccMacrostate0_Start"]+"-"+row["OccMacrostate1_Start"]+"-"+row["OccMacrostate2_Start"]+" "+str(TestProject.dt*row["OccMacrostate0_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate1_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate2_Stop"])+"-"+str(TestProject.dt*(row["OccMacrostate2_Stop"]+row["OccMacrostate2_Dwell"])))
            elif start == "2" and end == "2'":
                print(str(row["TrajNum"])+" FORWARD "+row["OccMacrostate0_Start"]+"-"+row["OccMacrostate1_Start"]+"-"+row["OccMacrostate2_Start"]+" "+str(TestProject.dt*row["OccMacrostate0_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate1_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate2_Stop"])+"-"+str(TestProject.dt*(row["OccMacrostate2_Stop"]+row["OccMacrostate2_Dwell"])))
        ...
        for transr in trans.iterrows():
            row = transr[1]
            start = row["OccMacrostate0_Start"]
            end = row["OccMacrostate1_Start"]

            if (start[0] == end[0]):
                if start.endswith("'"):
                    print(str(row["TrajNum"])+" BACK "+row["OccMacrostate0_Start"]+"-"+row["OccMacrostate1_Start"]+" "+str(TestProject.dt*row["OccMacrostate0_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate1_Stop"])+"-"+str(TestProject.dt*(row["OccMacrostate1_Stop"]+row["OccMacrostate1_Dwell"])))
                elif end.endswith("'"):
                    print(str(row["TrajNum"])+" FORWARD "+row["OccMacrostate0_Start"]+"-"+row["OccMacrostate1_Start"]+" "+str(TestProject.dt*row["OccMacrostate0_Stop"])+"-"+str(TestProject.dt*row["OccMacrostate1_Stop"])+"-"+str(TestProject.dt*(row["OccMacrostate1_Stop"]+row["OccMacrostate1_Dwell"])))
